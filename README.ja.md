...
設定、認証情報、診断、Windows Service管理には`githubie.exe`を使います。MCP ClientにはRepository一覧、承認付きRepository登録、Repository Status／Description、GitHub Actions workflow起動・run取得、fetch／pull／push、承認付き履歴訂正、Branch、Issue、Pull Request、Tag、Release、Version取得の46個の型付きToolを公開します。詳細は[コマンド](COMMANDS.ja.md)を参照してください。
...
MCP EndpointはLoopbackだけで待ち受けます。Personal Access Tokenを設定やMCP Clientへ保存せず、Repository登録承認後の最前面画面または`githubie.exe auth set`で登録してください。どちらのGUI経路でも、Token画面には入力前に登録元Project名と登録対象Repository URLを表示します。対象Repositoryだけに限定したFine-grained PATへ`Contents: Read and write`と`Pull requests: Read and write`を付与する方法を推奨します。詳細は[セキュリティ](SECURITY.ja.md)を参照してください。
