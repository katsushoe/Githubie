...
| `branch_source_invalid` | 作成元がnull・空文字・空白 | `source`へBranch名または完全な40桁コミットSHAを明示する。暗黙の既定値はない |
| `branch_source_not_found` | 指定した作成元コミットが対象Repositoryに存在しない | Repositoryと完全なSHAを確認する。Branchは未作成 |
