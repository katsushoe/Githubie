...
public sealed class BranchSourceCliTests
{
    [Theory]
    [InlineData("main", 0)]
    [InlineData("release/base", 0)]
    ...
    public async Task McpCall_ForwardsExplicitSourceAndServerOutcome(string? source, int expectedExit)
    {
        ...
        try
        {
            var arguments = new Dictionary<string, string> { ["repository"] = "sample", ["branch"] = "develop" };
            ...
            var execution = CliApplication.RunAsync(
                ["--config", config, "mcp", "call", "github_branch_create", JsonSerializer.Serialize(arguments)],
                output, error, timeout.Token);
            ...
            parameters.GetProperty("name").GetString().Should().Be("github_branch_create");
            var actual = parameters.GetProperty("arguments");
