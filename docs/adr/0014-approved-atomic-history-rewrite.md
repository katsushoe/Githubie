...
Every real rewrite requires an out-of-band Windows desktop approval. The LocalSystem service obtains the active console user's token and launches `Githubie.ApprovalPrompt` directly on `winsta0\\default` with `CreateProcessAsUser`; Task Scheduler is not used because deleting a one-shot task after requesting its run can race with process creation. The service exchanges a secret-free request over a per-request ACL-restricted named pipe. Denial, timeout, missing interactive session, launch failure, and protocol failure all fail closed and are logged as distinct events.
...
- Direct interactive-session launch requires the service account privileges provided to LocalSystem. Failure to locate the active console session or create the process returns immediately instead of being reported as an approval timeout.
